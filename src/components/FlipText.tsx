...
export function FlipText({
  ...
  useEffect(() => {
    // Trigger on mount after a delay to let global preloader exit first
    const timer = setTimeout(() => setIsMounted(true), mountDelay);
    return () => clearTimeout(timer);
  }, [mountDelay]);

  const trigger = isMounted || isInView;
            ...
            {chars.map((char, charIndex) => {
              ...
              return (
                <motion.span
                  ...
                  animate={trigger ? { rotateX: 0, y: 0, opacity: 1 } : { rotateX: -90, y: 15, opacity: 0 }}
                  transition={{
                    duration: duration,
